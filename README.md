...
<a id="application"></a>
### Application
A local application can be generated by running the below code from the inner folder 'BTS_Flight_Forecast' that contains 'utils' folder
```bash
conda activate BTS_Flight_Forecast
...
<a id="testing"></a>
### Running Tests
For running the tests, execute the following commands from the he inner folder 'BTS_Flight_Forecast' that contains 'tests' folder
```bash
...
1. **Using tool's GUI**:
    ...
    - Click retrain.
2. **Using script**:Update the START_YEAR and END_YEAR values in BTS_Flight_Forecast/scripts/reset_and_setup.py. Run the below script from the inner folder 'BTS_Flight_Forecast' that contains 'scripts' folder
   ```bash
    conda activate BTS_Flight_Forecast
